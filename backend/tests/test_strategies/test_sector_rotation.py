...
async def test_params():
    s = SectorRotationStrategy(
        params={"lookback_weeks": 8, "min_strength_score": 50},
    )
...
async def test_volatility_normalized_score_in_indicators():
    ...
    assert ind["volatility"] > 0, "uptrend should have vol"
...
async def test_low_vol_gets_higher_normalized_score():
    ...
    sig_low = await s.analyze(_build(low_vol_closes), "LOW")
    sig_high = await s.analyze(
        _build(high_vol_closes), "HIGH",
    )
    assert sig_low.indicators["volatility_normalized_score"] > (
        sig_high.indicators["volatility_normalized_score"]
    ), "low-vol asset should score higher after normalisation"


@pytest.mark.asyncio
async def test_compute_volatility_short_series():
    """_compute_volatility must return 0 for too-short series."""
...
async def test_severe_sell_has_high_confidence():
    """Strong downtrend triggers severe sell with conf >= 0.50."""
    s = SectorRotationStrategy(
        params={"min_strength_score": 20},
    )
    df = _make_df(100, trend="down")
...
async def test_moderate_sell_has_lower_confidence():
    """Mild decline triggers moderate sell with reduced conf."""
    s = SectorRotationStrategy(
        params={"min_strength_score": 20},
    )
