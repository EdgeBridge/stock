...
class TestPipelineConfigExtendedHours:
    ...
    def test_defaults(self):
        ...
        assert cfg.extended_hours_max_position_pct == 0.03
        assert cfg.extended_hours_slippage_multiplier == 3.0
        assert cfg.extended_hours_fill_probability == 0.70
        assert cfg.extended_hours_min_confidence == 0.70
...
class TestExtendedHoursSLCheck:
    """Test _check_extended_hours_sl (gap-down defense)."""
...
class TestExtendedHoursBuys:
    ...
    def _make_stock_data(self, symbol, n=3, base_price=100):
        df = pd.DataFrame({
            "open": [base_price] * n,
    ...
    def test_extended_buy_high_confidence(self):
        """High-confidence signal → extended hours buy at next open."""
        bt = self._make_pipeline()
        ...
        assert pos.quantity * pos.avg_price < 100000 * 0.04  # some buffer
