...
Buy: ROC5 > threshold AND ROC10 > threshold AND volume confirmed
Sell: ROC5 < -threshold AND ROC10 < -threshold (momentum reversal)
"""
...
class CISMomentumStrategy(BaseStrategy):
    ...
    async def analyze(self, df: pd.DataFrame, symbol: str) -> Signal:
        ...
        # BUY: strong upward momentum + volume confirmation
        if (roc5 > self._roc_short_buy
            ...
            return Signal(
                ...
                reason=f"Momentum up: ROC5={roc5:+.1f}%, ROC10={roc10:+.1f}%, Vol={volume_ratio:.1f}x",
        ...
        return self._hold(f"Momentum unclear: ROC5={roc5:+.1f}%, ROC10={roc10:+.1f}%")
    ...
    def set_params(self, params: dict) -> None:
        for key in [
            ...
            "roc_short_sell", "roc_long_sell", "volume_ratio_threshold",
        ]:
            if key in params:
