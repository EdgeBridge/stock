...
class SectorRotationStrategy(BaseStrategy):
    ...
    def _compute_volatility(
        ...
        if len(closes) < window + 1:
            return 0.0
        log_ret = np.log(
            closes.iloc[-window:] / closes.iloc[-window:].shift(1),
        ).dropna()
        if len(log_ret) < 2:
            return 0.0
    ...
    async def analyze(  # noqa: C901
        ...
        # --- SELL (relaxed) ------------------------------------------
        # Severe weakness: both score deeply negative AND short-term
        # return well below threshold → full sell.
        # Moderate weakness: score < 0 but short return only mildly
        # negative → reduced-confidence sell so other signals can
        # override.
        ...
        return self._hold(
            f"Strength={strength_score:.0f}, below threshold",
        )
