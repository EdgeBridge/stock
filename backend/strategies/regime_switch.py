    ...
    async def analyze(self, df: pd.DataFrame, symbol: str) -> Signal:
        ...
        if above_ma and confirmed:
            ...
            return Signal(
                ...
                reason=f"Bull regime: price above MA ({confirmed_days}d confirmed)",
        ...
        if not above_ma and confirmed:
            ...
            return Signal(
                ...
                confidence=min(confidence, 0.95),
                strategy_name=self.name,
                reason=f"Bear regime: price below MA ({confirmed_days}d confirmed)",
                suggested_price=price,
                indicators=indicators,
            )

        return self._hold(f"Regime transition, {confirmed_days}d {'above' if above_ma else 'below'} MA")
