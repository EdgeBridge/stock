...
def _warn_if_disabled_empty(market: str, disabled: list[str]) -> None:
    ...
    An empty list means all 17 strategies will be active for that market.
    ...
    if not disabled:
        logger.warning(
            ...
            " — all 17 strategies are active for %s. Verify this is intentional"
            " before live trading.",
            market,
            market,
        )
...
async def lifespan(app: FastAPI):
    ...
    # initial deploy — pre-alembic columns with no dedicated migration).
